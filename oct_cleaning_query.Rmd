...
#write.csv(data, "~/BasicInformation.csv",row.names = F)
...
dim(data); length(unique(data$id_participant))
write.csv(data, "~/Primary_Self_Reported_Questionnaire.csv",row.names = F)

#### Blood sample ####
##### nrow=2702, n_id=1677, 1677 ppts had >=1 blood draw ####
DATA=read.csv("~/PROMOTEDatabase_DATA_2023-08-17_1144.csv")
...
table(n_miss)
dim(data); length(unique(data$id_participant))
write.csv(data, "~/BloodSample.csv",row.names = F)
...
dim(data); length(unique(data$id_participant))
write.csv(data, "~/FunctionalTest.csv",row.names = F)
...
dim(data); length(unique(data$id_participant))
write.csv(data, "~/EDSS.csv",row.names = F)
...
write.csv(long, "~/RelapseHistory.csv",row.names = F)
...
dim(long); length(unique(long$id_participant))
write.csv(long, "~/DMTHistory.csv",row.names = F)
...
dim(long); length(unique(long$id_participant))
write.csv(long, "~/PDDSHistory.csv",row.names = F)
...
dim(long); length(unique(long$id_participant))
write.csv(long, "~/MSRSRHistory.csv",row.names = F)


###PROMISE###
data=read.csv("~/PROMOTELongitudinalQ_DATA_2023-08-17_1210.csv")
...
write.csv(physical, "~/promis_physical_function.csv",row.names = F)
...
write.csv(cog, "~/promis_cognitionn.csv",row.names = F)
...
write.csv(dep, "~/promis_depression.csv",row.names = F)
